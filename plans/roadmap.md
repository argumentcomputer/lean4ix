...
| V3 recursive projection staging delta | The recursive-singleton readiness mismatch is now isolated at the generated-minor boundary. Projection syntax has a selector minor that binds fields followed by the exact generated induction-hypothesis telescope and is stable under universe, lift, and term substitution. The recursor program obtains that telescope from the specialized checked minor, and `MinorsWF` plus every recursor-typing consumer now state their contract against that exact generated domain. The former field-only type survives only as a proved normalization used through one named compatibility theorem for the current `VStructureView`; the common-spine proof no longer rederives its field normalization inline. Remaining work is to remove that compatibility equation, type the selector under arbitrary generated IH binders, and beta-reduce those ignored binders in the exact iota proof. |
...
- **V3 — `addDecl.WF`, inductive case.** Generalize what the concrete replay
  ...
  remaining projection proof must account for the
