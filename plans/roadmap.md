...
- **V3 — `addDecl.WF`, inductive case.** Generalize what the concrete replay
  ...
  unconditionally. A generic successor theorem now advances the minor prefix
  across a dependent field from exact canonical equations for the earlier
  projectors; registered generated-iota rules derive that exact prefix from
  the corresponding canonical capture-spine typing. The remaining
  selecting-minor obligation is therefore the construction of those exact
  capture spines from checked generation, after which the bounded
  minor/program induction can iterate. The primitive
  ...
  completeness is now unconditional; next construct the generated-rule
  capture spines that feed the dependent minor/program successor, iterate that
  prefix induction, then derive the persistent eta-rule certificate before
  closing `addDecl.WF`.
