...
- **V3 — `addDecl.WF`, inductive case.** Generalize what the concrete replay
  ...
  Primitive readiness now derives exact final-constructor classification,
  completed Theory constructor heads, and both current and future-model cached
  parameter-count agreement. The remaining family-projection and
  structure-eta fields require repairing the host metadata boundary described
  in the status snapshot. Next derive selecting-minor and persistent eta-rule
