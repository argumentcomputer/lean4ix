...
- **V3 — `addDecl.WF`, inductive case.** Generalize what the concrete replay
  ...
  capture contract. The remaining eta obligation is deliberately persistent:
  `VStructEta.WF` requires reconstruction under every arbitrary `VEnv.LE`
  extension, whereas the completed current-model proof still requires target
  `VEnv.WF`. The primitive
  ...
  completeness is now unconditional; next lift reconstruction through the
  registry's weak arbitrary-extension contract and derive the persistent
  eta-rule certificate before closing `addDecl.WF`.
