...
Snapshot date: 2026-08-24. Counts are derived from the compiled audit and
...
That condition does **not** close R5 as previously claimed. If the matched
...
Repair R5 before further proof search. The accepted contract must provide
one of the following without becoming a confluence oracle: a derived
match-stability/non-function law for saturated generated pattern arguments;
an explicit eta/structure critical-pair compatibility certificate; or a
reduction/`NormalEq` redesign that represents those joins. Reuse the landed
match/spine inversion, `Check.OK` transport, level-list congruence, and
closed-template RHS congruence after that statement decision. Coordinate the
structure branch with D4 rather than declaring R5 independent of it.
...
- **V3 — `addDecl.WF`, inductive case.** Generalize what the concrete replay
  ...
  phase is now generic. The former whole-run normalization premise is derived
  from exact source-ordered family/constructor candidate-observer contracts;
  these recursive observer calls are the only operational facts absent from
  the public run. Generic ordinary/nested semantic transactions
  ...
  projection artifacts, and cross-safety ordering. A per-structure artifact
  constructs the final `StructureEtaArtifact` from a projection artifact and
  the descriptor's persistent `WF` certificate. A global coverage object now
  ...
  syntax plus a retained source-indexed semantic normalization run now derives
  the exact raw Theory family and constructor inventory. Checked generation
  derives the canonical recursor name internally, and that inventory turns
  coherent ordinary traces into the complete primitive transaction. Next
  assemble the retained semantic run from the public execution, prove that
  the no-op nested phase preserves the canonical source list, supply primitive
  readiness, derive selecting-minor and persistent eta-rule certificates, and
  discharge the remaining recursive candidate observers before closing
  `addDecl.WF`.
