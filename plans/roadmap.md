...
- **V3 — `addDecl.WF`, inductive case.** Generalize what the concrete replay
  ...
  Constructor success is retained as an antecedent because recursive Pi
  observers use its root type-check evidence. Canonical Bool/Nat observers
  are now derived from family validation, the real family insertion fold,
  ...
  Primitive candidate normalization is likewise closed. Next derive the
  nonprimitive selecting-minor and persistent eta-rule certificates and its
  remaining source-ordered recursive candidate observers before closing
  `addDecl.WF`.
