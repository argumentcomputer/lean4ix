...
| Upstream intake audit | Refreshed and implemented through UP5 plus the first five UP6 primitive slices on 2026-08-26 against upstream `e0e3f6bc`, which is still the fork's merge base. UP0-UP4 manually adapt PR #45's truthful domains/models, PR #44's bounded loose-bvar and safe-checker path, and PR #46's audit evidence. UP5 manually adapts the three accepted `differential` commits. UP6 now has its committed body-first foundation plus direct, live `Nat.add`, `Nat.pred`, `Nat.sub`, `Nat.mul`, and `Nat.pow` checker/conservation certificates. All five dependency pins exclude the generic primitive boundary and record exactly six inherited upstream proof dependencies. The elementary recurrence spine is complete, and the retained binary reflection contract carries the typing evidence needed when one certified primitive applies another to an arbitrary recursive result. PR #32 still does **not** prove generic `checkPrimitiveDef.WF`; the remaining primitive families, final exhaustive dispatch, and V4 disposition remain open. PR #43 and PR #27 remain explicit non-imports. |
...
This completes the elementary Nat recurrence spine. The next UP6 cluster is
`Nat.beq`, `Nat.ble`, shifts, and the small bitwise wrappers, beginning with a
fresh dependency/accepted-domain audit before selecting the next slice.
