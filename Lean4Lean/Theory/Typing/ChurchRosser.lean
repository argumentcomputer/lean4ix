...
class Params where
  ...
  structEta_weakN_inv :
    env.structEtas rule →
    (∀ level ∈ levels, level.WF univs) →
    levels.length = rule.uvars →
    params.length = rule.nparams →
    OnCtx Γ' (env.IsType univs) →
    Ctx.LiftN n k Γ Γ' →
    IsDefEq env univs Γ' (e₁.liftN n k) (e₂.liftN n k)
      (rule.structureType levels params) →
    ∃ levels' params' resultLevel,
      levels'.length = rule.uvars ∧
      params'.length = rule.nparams ∧
      (∀ level ∈ levels', level.WF univs) ∧
      SpineWF env univs Γ (rule.familyType.instL levels') params'
        (.sort resultLevel) ∧
      IsDefEq env univs Γ e₁ e₂
        (rule.structureType levels' params')
  /-- A registered structure-family application is disjoint from the two
  rigid type heads used by head standardization.  These are primitive head
  coverage facts, not confluence or equality conclusions. -/
  structEta_sort_disjoint :
    env.structEtas rule →
    (∀ level ∈ levels, level.WF univs) →
    levels.length = rule.uvars →
    params.length = rule.nparams →
    ¬ IsDefEqU env univs Γ
      (rule.structureType levels params) (.sort u)
  structEta_forallE_disjoint :
    env.structEtas rule →
    (∀ level ∈ levels, level.WF univs) →
    levels.length = rule.uvars →
    params.length = rule.nparams →
    ¬ IsDefEqU env univs Γ
      (rule.structureType levels params) (.forallE A B)
  /-- Function-head typing is reflected through weakening.  This is the
  primitive head-inversion coverage needed when a residual structure-eta
  equality is closed pointwise under ordinary function eta. -/
  forallE_weakN_inv :
    OnCtx Γ' (env.IsType univs) →
    Ctx.LiftN n k Γ Γ' →
    HasType env univs Γ' (f.liftN n k) (.forallE A B) →
    ∃ A' B', HasType env univs Γ f (.forallE A' B')
...
/-- Residual conversion generated by a registered structure family.

The two seed constructors retain the *actual* left- or right-reconstruction
shape.  Their two equality payloads relate the majors and the complete
constructor spines (hence every retained parameter and generated projector
occurrence) at one descriptor.  The final two premises compose ordinary
conversion around that seed without forgetting it; unlike the former broad
base case, the reconstruction witness remains part of every `StructEq`.
`forallE` is the pointwise closure needed when structure eta appears beneath
ordinary function eta. -/
inductive StructEq : List VExpr → VExpr → VExpr → Prop where
  | etaL :
    env.structEtas rule →
    (∀ level ∈ levels, level.WF univs) →
    levels.length = rule.uvars →
    ...
    Γ ⊢ rule.rebuild levels params major₂ :
      rule.structureType levels params →
    Γ ⊢ major₁ ≡ major₂ : rule.structureType levels params →
    Γ ⊢ rule.rebuild levels params major₁ ≡
      rule.rebuild levels params major₂ : rule.structureType levels params →
    Γ ⊢ e₁ ≡ rule.rebuild levels params major₁ :
      rule.structureType levels params →
    Γ ⊢ major₂ ≡ e₂ : rule.structureType levels params →
    StructEq Γ e₁ e₂
  | etaR :
    env.structEtas rule →
    (∀ level ∈ levels, level.WF univs) →
    ...
    Γ ⊢ rule.rebuild levels params major₁ ≡
      rule.rebuild levels params major₂ : rule.structureType levels params →
    Γ ⊢ e₁ ≡ major₁ : rule.structureType levels params →
    Γ ⊢ rule.rebuild levels params major₂ ≡ e₂ :
      rule.structureType levels params →
    StructEq Γ e₁ e₂
  | forallE :
    Γ ⊢ f : .forallE A B →
    Γ ⊢ g : .forallE A B' →
    StructEq (A::Γ) (.app f.lift (.bvar 0)) (.app g.lift (.bvar 0)) →
    StructEq Γ f g
...
inductive NormalEq : List VExpr → VExpr → VExpr → Prop where
  ...
  | etaR :
    ...
    Γ ⊢ e' ≡ₚ .lam A e
  | structural : StructEq Γ e₁ e₂ → Γ ⊢ e₁ ≡ₚ e₂
  | proofIrrel :
    Γ ⊢ p : .sort .zero → Γ ⊢ h : p → Γ ⊢ h' : p →
    Γ ⊢ h ≡ₚ h'

variable! (hΓ : OnCtx Γ (env.IsType univs)) in
theorem StructEq.defeq (H : StructEq Γ e₁ e₂) : Γ ⊢ e₁ ≡ e₂ := by
  induction H with
  | etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
      _ hmajor₂ _ hrebuild₂ _ hrebuildEq hleft hright =>
    exact ⟨_, .trans hleft <| .trans hrebuildEq <| .trans
      (.structEta hreg hlevels hlevelsLength hparamsLength hparamsSpine
        hmajor₂ hrebuild₂) hright⟩
  | etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ _ hrebuild₁ _ _ hrebuildEq hleft hright =>
    exact ⟨_, .trans hleft <| .trans (.symm <|
      .structEta hreg hlevels hlevelsLength hparamsLength hparamsSpine
        hmajor₁ hrebuild₁) <| .trans hrebuildEq hright⟩
  | forallE hf hg _ ih =>
    have ⟨_, AB⟩ := hf.isType henv hΓ
    have ⟨⟨_, hA⟩, _⟩ := AB.forallE_inv henv
    have hΓ' : OnCtx (_ :: _) (env.IsType univs) :=
      ⟨hΓ, ⟨_, hA.hasType.1⟩⟩
    have ⟨_, he⟩ := ih hΓ'
    have hleft := IsDefEqU.symm ⟨_, .eta hf⟩
    have hmid := IsDefEq.lamDF hA he
    exact ⟨_, .transU_r henv hΓ hleft <| hmid.transU_l henv hΓ ⟨_, .eta hg⟩⟩

variable! (hΓ : OnCtx Γ (env.IsType univs)) in
theorem StructEq.not_sort_r (hu : u.WF univs) : ¬StructEq Γ e (.sort u)
  | .etaL hreg hlevels hlevelsLength hparamsLength _ _ _ _ _ _ _ _ hright
  | .etaR hreg hlevels hlevelsLength hparamsLength _ _ _ _ _ _ _ _ hright =>
    structEta_sort_disjoint hreg hlevels hlevelsLength hparamsLength
      (hright.hasType.2.uniqU henv hΓ (HasType.sort hu))
  | .forallE _ hg _ =>
    ((HasType.sort hu).uniqU henv hΓ hg).sort_forallE_inv henv hΓ

variable! (hΓ : OnCtx Γ (env.IsType univs)) in
theorem StructEq.not_forallE_r
    (hAB : Γ ⊢ .forallE A B : .sort u) : ¬StructEq Γ e (.forallE A B)
  | .etaL hreg hlevels hlevelsLength hparamsLength _ _ _ _ _ _ _ _ hright
  | .etaR hreg hlevels hlevelsLength hparamsLength _ _ _ _ _ _ _ _ hright =>
    structEta_sort_disjoint hreg hlevels hlevelsLength hparamsLength
      (hright.hasType.2.uniqU henv hΓ hAB)
  | .forallE _ hg _ =>
    (hAB.uniqU henv hΓ hg).sort_forallE_inv henv hΓ

variable! (hΓ : OnCtx Γ (env.IsType univs)) in
theorem StructEq.symm (H : StructEq Γ e₁ e₂) : StructEq Γ e₂ e₁ := by
  induction H with
  | etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
      hleft hright =>
    exact .etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₂ hmajor₁ hrebuild₂ hrebuild₁ hmajorEq.symm
        hrebuildEq.symm hright.symm hleft.symm
  | etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
      hleft hright =>
    exact .etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₂ hmajor₁ hrebuild₂ hrebuild₁ hmajorEq.symm
        hrebuildEq.symm hright.symm hleft.symm
  | forallE hf hg _ ih =>
    have ⟨_, AB⟩ := hf.isType henv hΓ
    exact .forallE hg hf (ih ⟨hΓ, (AB.forallE_inv henv).1⟩)

theorem StructEq.weakN (W : Ctx.LiftN n k Γ Γ')
    (H : StructEq Γ e₁ e₂) :
    StructEq Γ' (e₁.liftN n k) (e₂.liftN n k) := by
  ...
  | etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
    ...
    have hmajor₂ := hmajor₂.weakN henv W
    have hrebuild₁ := hrebuild₁.weakN henv W
    have hrebuild₂ := hrebuild₂.weakN henv W
    have hmajorEq := hmajorEq.weakN henv W
    have hrebuildEq := hrebuildEq.weakN henv W
    have hleft := hleft.weakN henv W
    have hright := hright.weakN henv W
    simp only [VStructEta.structureType_liftN] at hmajor₁ hmajor₂ hmajorEq hright
    simp only [VStructEta.rebuild_liftN _ _ _ _ hparamsLength,
      VStructEta.structureType_liftN] at hrebuild₁ hrebuild₂ hrebuildEq hleft
    exact .etaL hreg hlevels hlevelsLength (by simpa using hparamsLength)
      hparamsSpine hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq
        hrebuildEq hleft hright
  | etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
      hleft hright =>
    have hparamsSpine := hparamsSpine.weakN henv W
    ...
    have hleft := hleft.weakN henv W
    have hright := hright.weakN henv W
    simp only [VStructEta.structureType_liftN] at hmajor₁ hmajor₂ hmajorEq hleft
    simp only [VStructEta.rebuild_liftN _ _ _ _ hparamsLength,
      VStructEta.structureType_liftN] at hrebuild₁ hrebuild₂ hrebuildEq hright
    exact .etaR hreg hlevels hlevelsLength (by simpa using hparamsLength)
      hparamsSpine hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq
        hrebuildEq hleft hright
  | forallE hf hg _ ih =>
    refine .forallE (hf.weakN henv W) (hg.weakN henv W) ?_
    simpa [liftN, lift_liftN'] using ih W.succ

variable! (h₀ : Γ₀ ⊢ e₀ : A₀) in
theorem StructEq.instN (W : Ctx.InstN Γ₀ e₀ A₀ k Γ₁ Γ)
    (H : StructEq Γ₁ e₁ e₂) :
    StructEq Γ (e₁.inst e₀ k) (e₂.inst e₀ k) := by
  ...
  | etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
    ...
    have hmajor₂ := hmajor₂.instN henv W h₀
    have hrebuild₁ := hrebuild₁.instN henv W h₀
    have hrebuild₂ := hrebuild₂.instN henv W h₀
    have hmajorEq := hmajorEq.instN henv h₀ W
    have hrebuildEq := hrebuildEq.instN henv h₀ W
    have hleft := hleft.instN henv h₀ W
    have hright := hright.instN henv h₀ W
    simp only [VStructEta.structureType_instN] at hmajor₁ hmajor₂ hmajorEq hright
    simp only [VStructEta.rebuild_instN _ _ _ _ _ hparamsLength,
      VStructEta.structureType_instN] at hrebuild₁ hrebuild₂ hrebuildEq hleft
    exact .etaL hreg hlevels hlevelsLength (by simpa using hparamsLength)
      hparamsSpine hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq
        hrebuildEq hleft hright
  | etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
    ...
    have hleft := hleft.instN henv h₀ W
    have hright := hright.instN henv h₀ W
    simp only [VStructEta.structureType_instN] at hmajor₁ hmajor₂ hmajorEq hleft
    simp only [VStructEta.rebuild_instN _ _ _ _ _ hparamsLength,
      VStructEta.structureType_instN] at hrebuild₁ hrebuild₂ hrebuildEq hright
    exact .etaR hreg hlevels hlevelsLength (by simpa using hparamsLength)
      hparamsSpine hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq
        hrebuildEq hleft hright
  | forallE hf hg _ ih =>
    refine .forallE (hf.instN henv W h₀) (hg.instN henv W h₀) ?_
    simpa [inst, lift_instN_lo] using ih W.succ

variable! (hΓ : OnCtx Γ (env.IsType univs)) in
theorem StructEq.app (H : StructEq Γ f g)
    (hf : Γ ⊢ f : .forallE A B) (ha : Γ ⊢ a : A) :
    StructEq Γ (.app f a) (.app g a) := by
  cases H with
  | etaL hreg hlevels hlevelsLength hparamsLength _ _ _ _ _ _ _ hleft _ =>
    exact (structEta_forallE_disjoint hreg hlevels hlevelsLength
      hparamsLength (hleft.hasType.1.uniqU henv hΓ hf)).elim
  | etaR hreg hlevels hlevelsLength hparamsLength _ _ _ _ _ _ _ hleft _ =>
    exact (structEta_forallE_disjoint hreg hlevels hlevelsLength
      hparamsLength (hleft.hasType.1.uniqU henv hΓ hf)).elim
  | forallE hf' hg' h =>
    have ⟨⟨_, hA⟩, _⟩ := (hf.uniqU henv hΓ hf').forallE_inv henv hΓ
    simpa [inst, inst_lift, instN_bvar0] using
      h.instN (hA.defeq ha) .zero

private theorem SpineWF.defeqDFC_early
    (W : IsDefEqCtx env univs Γ₀ Γ₁ Γ₂) :
    ∀ {A es B}, SpineWF env univs Γ₁ A es B →
      SpineWF env univs Γ₂ A es B
  | _, [], _, .nil => .nil
  | _, _ :: _, _, .cons he hrest =>
    .cons (he.defeqDFC henv W) (SpineWF.defeqDFC_early W hrest)

variable! (H₀ : OnCtx Γ₀ (IsType env univs)) in
theorem StructEq.defeqDFC (W : IsDefEqCtx env univs Γ₀ Γ₁ Γ₂)
    (H : StructEq Γ₁ e₁ e₂) : StructEq Γ₂ e₁ e₂ := by
  induction H generalizing Γ₂ with
  | etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
      hleft hright =>
    exact .etaL hreg hlevels hlevelsLength hparamsLength
      (hparamsSpine.defeqDFC_early W)
      (.defeqDFC henv W hmajor₁) (.defeqDFC henv W hmajor₂)
      (.defeqDFC henv W hrebuild₁) (.defeqDFC henv W hrebuild₂)
      (.defeqDFC henv W hmajorEq) (.defeqDFC henv W hrebuildEq)
      (.defeqDFC henv W hleft) (.defeqDFC henv W hright)
  | etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
      hleft hright =>
    exact .etaR hreg hlevels hlevelsLength hparamsLength
      (hparamsSpine.defeqDFC_early W)
      (.defeqDFC henv W hmajor₁) (.defeqDFC henv W hmajor₂)
      (.defeqDFC henv W hrebuild₁) (.defeqDFC henv W hrebuild₂)
      (.defeqDFC henv W hmajorEq) (.defeqDFC henv W hrebuildEq)
  ...
  | forallE hf hg _ ih =>
    have ⟨⟨_, hA⟩, _⟩ :=
      let ⟨_, h⟩ := hf.isType henv (W.isType' H₀); h.forallE_inv henv
    exact .forallE (.defeqDFC henv W hf) (.defeqDFC henv W hg)
      (ih (W.succ hA))
...
theorem NormalEq.symm (H : Γ ⊢ e1 ≡ₚ e2) : Γ ⊢ e2 ≡ₚ e1 := by
  ...
  | structural h => exact .structural (h.symm hΓ)
...
theorem NormalEq.weakN (W : Ctx.LiftN n k Γ Γ') (H : Γ ⊢ e1 ≡ₚ e2) :
  ...
  | etaR h1 _ ih =>
    ...
    have := ih W.succ
    simp [liftN] at this; rwa [lift_liftN']
  | structural h => exact .structural (h.weakN W)
  | proofIrrel h1 h2 h3 =>
...
theorem NormalEq.instN (W : Ctx.InstN Γ₀ e₀ A₀ k Γ₁ Γ) (H : Γ₁ ⊢ e1 ≡ₚ e2) :
  ...
  | etaR h1 _ ih =>
    ...
    simpa [inst, lift_instN_lo] using ih W.succ
  | structural h => exact .structural (h.instN h₀ W)
...
theorem NormalEq.defeqDFC (W : IsDefEqCtx env univs Γ₀ Γ₁ Γ₂)
  ...
  | structural h => exact .structural (h.defeqDFC H₀ W)
  | proofIrrel h1 h2 h3 =>
...
private theorem hasType_app_bvar0_early
    (hΓ : OnCtx (A::Γ) (IsType env univs))
    (H : A :: Γ ⊢ e.lift.app (bvar 0) : B) :
    ∃ B', Γ ⊢ e : .forallE A B' := by
  ...
  have heta := c1.eta
  rw [show A.lift.lam (e.lift.lift.app (bvar 0)) =
      (A.lam (e.lift.app (bvar 0))).lift by
    simp [VExpr.liftN, liftN'_liftN_lo, liftN_liftN]] at heta
  have ⟨_, f1⟩ := (IsDefEqU.weakN_iff henv hΓ .one).1 ⟨_, heta⟩
  have ⟨⟨_, f2⟩, _, f3⟩ := f1.hasType.1.lam_inv henv hΓ.1
  exact ⟨_, (HasType.lam f2 f3).defeqU_l henv hΓ.1 ⟨_, f1⟩⟩

variable! (hΓ₀ : OnCtx Γ₀ (IsType env univs)) in
theorem StructEq.weakN_inv_DFC
    (W : Ctx.LiftN n k Γ Γ₂)
    (W₂ : IsDefEqCtx env univs Γ₀ Γ₁ Γ₂)
    (H : StructEq Γ₁ (e₁.liftN n k) (e₂.liftN n k)) :
    StructEq Γ e₁ e₂ := by
  generalize eq0 : Γ₁ = Γ₁' at H
  generalize eq1 : e₁.liftN n k = e₁' at H
  generalize eq2 : e₂.liftN n k = e₂' at H
  revert W₂
  induction H generalizing Γ Γ₁ Γ₂ e₁ e₂ k with
  | etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
      hleft hright =>
    intro W₂
    ...
    have hseed : Γ₁ ⊢ e₁.liftN n k ≡ e₂.liftN n k :
        _ := .trans hleft <| .trans hrebuildEq <| .trans
      (.structEta hreg hlevels hlevelsLength hparamsLength hparamsSpine
        hmajor₂ hrebuild₂) hright
    obtain ⟨levels', params', resultLevel', hlevelsLength', hparamsLength',
        hlevels', hparamsSpine', h'⟩ :=
      structEta_weakN_inv hreg hlevels hlevelsLength hparamsLength hΓ₂ W
    ...
    exact .etaL hreg hlevels' hlevelsLength' hparamsLength' hparamsSpine'
      hmajor hmajor hrebuild hrebuild hmajor hrebuild
        (.trans h' heta.symm) hmajor
  | etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
      hleft hright =>
    intro W₂
  ...
  | forallE hf hg _ ih =>
    ...
    have hΓ₁ := W₂.isType' hΓ₀
    have ⟨⟨_, hA⟩, _, _⟩ :=
      let ⟨_, h⟩ := hf.isType henv hΓ₁; h.forallE_inv henv
    have hf' := hf.defeqDFC henv W₂
    have hΓ₂ := (W₂.symm henv).isType' hΓ₀
    obtain ⟨A₀, B₀, hf₀⟩ := forallE_weakN_inv hΓ₂ W hf'
    have ⟨⟨_, hAeq⟩, _⟩ :=
      (hf'.uniqU henv hΓ₂ (hf₀.weakN henv W)).forallE_inv henv hΓ₂
    have hAeq' := hAeq.defeqDFC henv (W₂.symm henv)
    have hbody := ih (e₁ := e₁.lift.app (.bvar 0))
      (e₂ := e₂.lift.app (.bvar 0)) W.succ rfl
      (by simp [liftN, lift_liftN']) (by simp [liftN, lift_liftN'])
      (W₂.succ hAeq')
    have hΓ := hΓ₂.weakN_inv henv W
    have ⟨_, hfunType⟩ := hf₀.isType henv hΓ
    have ⟨⟨_, hA₀⟩, _⟩ := hfunType.forallE_inv henv
    have hΓ' : OnCtx (_ :: Γ) (env.IsType univs) :=
      ⟨hΓ, ⟨_, hA₀.hasType.1⟩⟩
    obtain ⟨_, hg₀⟩ := hasType_app_bvar0_early hΓ'
      (hbody.defeq hΓ').choose_spec.hasType.2
    exact .forallE hf₀ hg₀ hbody
...
theorem NormalEq.weakN_inv_DFC (W : Ctx.LiftN n k Γ Γ₂) (W₂ : IsDefEqCtx env univs Γ₀ Γ₁ Γ₂)
  ...
  | structural h =>
    subst eq1
    subst eq2
    exact .structural (h.weakN_inv_DFC hΓ₀ W W₂)
  | proofIrrel h1 h2 h3 =>
    subst eq1; subst eq2
    have h1' := h1.defeqDFC henv W₂
...
variable! (hΓ : OnCtx Γ (IsType env univs)) in
theorem StructEq.trans_right (H : StructEq Γ e₁ e₂)
    (h : Γ ⊢ e₂ ≡ e₃) : StructEq Γ e₁ e₃ := by
  ...
  | etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
    ...
    exact .etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
        hleft (hright.transU_l henv hΓ h)
  | etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor₁ hmajor₂ hrebuild₁ hrebuild₂ hmajorEq hrebuildEq
      hleft hright =>
    exact .etaR hreg hlevels hlevelsLength hparamsLength hparamsSpine
  ...
  | forallE hf hg _ ih =>
    ...
    have hΓ' : OnCtx (_ :: _) (env.IsType univs) :=
      ⟨hΓ, ⟨_, hA.hasType.1⟩⟩
    have hfun := h.of_l henv hΓ hg
    have happ := IsDefEq.appDF (hfun.weakN henv .one) (.bvar .zero)
    exact .forallE hf hfun.hasType.2 (ih hΓ' ⟨_, happ⟩)

variable! (hΓ : OnCtx Γ (IsType env univs)) in
theorem StructEq.trans_left (h : Γ ⊢ e₀ ≡ e₁)
    (H : StructEq Γ e₁ e₂) : StructEq Γ e₀ e₂ :=
  ((H.symm hΓ).trans_right hΓ h.symm).symm hΓ
...
theorem NormalEq.trans (hΓ : OnCtx Γ (IsType env univs)) :
  ...
  | H1@(.etaR l1 ihl), .etaL r1 ihr => by
    ...
    | structural hs =>
      have hΓ' : OnCtx (_ :: _) (env.IsType univs) :=
        ⟨hΓ, ⟨_, hA.hasType.1⟩⟩
      have ⟨_, hse⟩ := hs.defeq hΓ'
      obtain ⟨_, hr⟩ := hasType_app_bvar0_early hΓ' hse.hasType.2
      exact .structural (.forallE l1 hr hs)
  | .structural hs, H2 => by
    exact .structural (hs.trans_right hΓ (H2.defeq hΓ))
  | .refl h, H2 => H2
  ...
  | H1, .structural hs => by
    exact .structural (hs.trans_left hΓ (H1.defeq hΓ))
...
variable! (hΓ : OnCtx Γ (IsType env univs)) in
theorem ParRedExt.beta_defeq (l : ParRedExt) (W : l.depth ≤ Γ.length)
    (H : Γ ⊢ l.apply ((lam A e').lift.app (bvar 0)) : T) :
    Γ ⊢ l.apply ((lam A e').lift.app (bvar 0)) ≡ l.apply e' := by
  induction l generalizing Γ T with
  | base =>
    simp only [apply] at H ⊢
    have ⟨_, _, hfun, harg⟩ := H.app_inv henv hΓ
    have ⟨⟨_, hA⟩, _, hbody⟩ := hfun.lam_inv henv hΓ
    have ⟨⟨_, hdom⟩, _⟩ :=
      ((hA.lam hbody).uniqU henv hΓ hfun).forallE_inv henv hΓ
    simpa [liftN, instN_bvar0] using
      (IsDefEq.toU (.beta hbody (hdom.symm.defeq harg)))
  | lift l ih =>
    let _ :: Γ' := Γ
    have ⟨_, H'⟩ := (VExpr.WF.weakN_iff henv hΓ .one).1 ⟨_, H⟩
    simpa [apply] using
      (ih hΓ.1 (Nat.le_of_succ_le_succ W) H').weakN henv (.one)
  | app l ih =>
    let _ :: Γ' := Γ
    obtain ⟨_, hfun⟩ := hasType_app_bvar0 hΓ H
    have heq := ih hΓ.1 (Nat.le_of_succ_le_succ W) hfun
    have happ := IsDefEq.appDF
      ((heq.of_l henv hΓ.1 hfun).weakN henv .one) (.bvar .zero)
    simpa [apply] using IsDefEq.toU happ
...
theorem ParRedExt.parRed_beta :
  ...
  · cases h1 with
    ...
    | structural hs =>
      have ⟨_, _, H1, H2⟩ := h2.app_inv henv hΓ
      have hsApp := hs.app hΓ H1 H2
      have hse := hs.defeq hΓ
      have hlam := (hse.of_l henv hΓ H1).hasType.2
      have ⟨⟨_, hA⟩, _, hbody⟩ := hlam.lam_inv henv hΓ
      have ⟨⟨_, hdom⟩, _⟩ :=
        ((hA.lam hbody).uniqU henv hΓ hlam).forallE_inv henv hΓ
      have hbeta : Γ ⊢ (lam A e').app a ≡ e'.inst a :=
        ⟨_, .beta hbody (hdom.symm.defeq H2)⟩
      exact ⟨_, .rfl, .structural (hsApp.trans_right hΓ hbeta)⟩
    | proofIrrel a1 a2 a3 =>
      have ⟨_, _, H1, H2⟩ := h2.app_inv henv hΓ
  ...
  | structural hs =>
    subst eq
    have ⟨_, heq⟩ := hs.defeq hΓ
    exact ⟨_, .rfl, .structural <|
      hs.trans_right hΓ (l.beta_defeq hΓ W heq.hasType.2)⟩
...
/-!
`StructEq` retains an oriented, registered eta seed plus the complete typed
constructor-spine congruence.  Consequently one parallel step at its right
endpoint is absorbed without erasing that seed.  This is the common typed
...
* beta and congruence steps inside the major, including every repeated
  projector occurrence, are the `beta` and `app` parallel cases;
* dependent later fields are transported by the common endpoint type in the
  resulting `IsDefEq` proof; and
* proof fields and Prop-valued structures use the same typed transport, with
  `NormalEq.proofIrrel` remaining available for the residual proof endpoints.

The helper is intentionally proved from `ParRed.defeq`, not assumed in
`Params`; the generic pattern interface therefore remains responsible for
the `.extra` overlap.
-/
variable! (hΓ : OnCtx Γ (IsType env univs)) in
theorem StructEq.parRed_right (H : StructEq Γ e₁ e₂)
...
theorem IsDefEq.church_rosser
  ...
  | structEta hreg hlevels hlevelsLength hparamsLength hparamsSpine
      hmajor hrebuild _ _ _ =>
    exact .normalEq hΓ <| .structural <|
      .etaL hreg hlevels hlevelsLength hparamsLength hparamsSpine
        hmajor hmajor hrebuild hrebuild hmajor hrebuild hrebuild hmajor
