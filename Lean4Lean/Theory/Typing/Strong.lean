...
/-- The environment has no registered structure-eta rule. This is the exact
condition under which the pre-structure-eta simultaneous-substitution proof
remains complete. -/
def NoStructEta (env : VEnv) : Prop := ∀ rule, ¬env.structEtas rule

/-- Simultaneous substitution for the strong judgment when the environment
has no registered structure-eta rules: the three-way left/right/cross bundle. -/
theorem IsDefEqStrong.substEq_of_noStructEta (hη : NoStructEta env)
    (henv : Ordered env) (hΓ : CtxStrong env U Γ) (hΓ₀ : CtxStrong env U Γ₀)
  ...
  | @constDF c ci ls ls' u _ h1 hWls hWls' hlen hFf hu hcty _ _ _ =>
    have hcl : (VExpr.instL ls ci.type).ClosedN 0 := (henv.closedC h1).instL
  ...
  | @eta Γ' A u B v e hu hv hA hB _ he _ _ ihA ihB _ ihe _ _ =>
    ...
    exact ⟨H_σ.trans (R.trans
      ...
      R, H_σ.trans R⟩
  | @structEta rule _ _ _ _ _ _ hreg => exact (hη rule hreg).elim
...
/-- Cross projection of `substEq_of_noStructEta`. -/
theorem IsDefEqStrong.subst_of_noStructEta (hη : NoStructEta env)
    (henv : Ordered env) (hΓ : CtxStrong env U Γ) (hΓ₀ : CtxStrong env U Γ₀)
    (W : Ctx.SubstEq env U Γ₀ σ σ' Γ) (H : env.IsDefEqStrong U Γ e1 e2 A) :
    env.IsDefEqStrong U Γ₀ (e1.subst σ) (e2.subst σ') (A.subst σ) :=
  (H.substEq_of_noStructEta hη henv hΓ hΓ₀ W).2.2

/-- Simultaneous substitution into weak definitional equality in an environment
without registered structure eta. This generalizes `IsDefEq.instDF`; removing
the premise requires a simultaneous-substitution theorem for `SpineWF` and
naturality of `VStructEta` projectors. -/
theorem IsDefEq.substDF_of_noStructEta (hη : NoStructEta env)
    (henv : Ordered env) (hΓ : OnCtx Γ (env.IsType U))
    (hΓ₀ : OnCtx Γ₀ (env.IsType U)) (W : Ctx.SubstEq env U Γ₀ σ σ' Γ)
    (H : env.IsDefEq U Γ e1 e2 A) :
    env.IsDefEq U Γ₀ (e1.subst σ) (e2.subst σ') (A.subst σ) :=
  ((H.strong henv hΓ).subst_of_noStructEta hη henv
    (.strong henv hΓ) (.strong henv hΓ₀) W).defeq
