...
@[simp] private theorem Expr.instantiate1'_const (n : Name)
    (ls : List Level) (a : Expr) (d : Nat) :
    (Expr.const n ls).instantiate1' a d = .const n ls := rfl
...
@[simp] private theorem Expr.instantiate1'_bvar (i : Nat)
    (subst : Expr) (d : Nat) :
    (Expr.bvar i).instantiate1' subst d =
      if i < d then .bvar i else if i = d then subst.liftLooseBVars' 0 d
      else .bvar (i - 1) := rfl
@[simp] private theorem Expr.instantiate1'_lit (l : Literal)
    (subst : Expr) (d : Nat) :
    (Expr.lit l).instantiate1' subst d = .lit l := rfl
@[simp] private theorem Expr.liftLooseBVars'_lit (l : Literal)
    (s d : Nat) :
    (Expr.lit l).liftLooseBVars' s d = .lit l := rfl

/-- A sandbox preserves state-independent semantic facts proved about its
result while restoring the incoming verifier state. -/
theorem TypeChecker.M.WF.sandbox_of_independent
    {c : TypeChecker.VContext} {s : TypeChecker.VState}
    {x : TypeChecker.M α} {Q : α → Prop}
    (h : TypeChecker.M.WF c s x fun a _ => Q a) :
    TypeChecker.M.WF c s (TypeChecker.M.sandbox x) fun a _ => Q a := by
  intro hwf a st hsandbox
  simp only [TypeChecker.M.sandbox] at hsandbox
  split at hsandbox
  · cases hsandbox
    obtain ⟨vs', _, _, _, hQ⟩ := h hwf _ _ (by assumption)
    exact ⟨s, rfl, .rfl, hwf, hQ⟩
  · cases hsandbox
...
theorem Condition.natEqDecideFn.instantiate
  ...
  obtain ⟨out, houtS, heq, -⟩ :=
    TrExprS.natNatLam_instantiate wf hctors hdecide a b
  refine ⟨out, ?_, heq⟩
  simpa [pure, Condition.natEqDecideFn, Condition.decide,
    Condition.ite, Condition.natEq, Lean.mkAppN,
    mkApp5, mkApp4, mkApp3, mkApp2, mkAppB, mkApp,
    Expr.instantiate1'_instantiate1'] using houtS
...
theorem Condition.natEqReflectedFn.instantiate
  ...
  obtain ⟨out, houtS, heq, -⟩ :=
    TrExprS.natNatLam_instantiate wf hctors hreflect a b
  refine ⟨out, ?_, heq⟩
  have htoDecClosed : Reflection.defn₂.toDec.looseBVarRange' = 0 := by
    rfl
  have hproofClosed : Condition.natEqReflectProof.looseBVarRange' = 0 := by
    rfl
  simpa [pure, Condition.natEqReflectedFn, Condition.natEq,
    Expr.instantiate1_eq_self htoDecClosed,
    Expr.instantiate1_eq_self hproofClosed,
...
theorem Condition.natEq_dec_application_eq_reflected
  ...
  obtain ⟨out, houtS, -, hall⟩ :=
    TrExprS.natNatLam_instantiate wf hctors hreflect a b
  refine ⟨out, ?_, hall heq⟩
  have htoDecClosed : Reflection.defn₂.toDec.looseBVarRange' = 0 := by
    rfl
  have hproofClosed : Condition.natEqReflectProof.looseBVarRange' = 0 := by
    rfl
...
theorem Condition.natBEq_application_eval
  ...
  have hab : Nat.beq a b = (a == b) := by
    ...
    simp [Nat.beq_eq]
  rw [← hab]
  exact Condition.reflectsNatNatBool_application_eval wf hprim.natBEq
    hctors hbeqC (hctors.natLitS a (Us := []) (Δ := [])).1
    (hctors.natLitS b (Us := []) (Δ := [])).1 hbeqS
        ...
        | app hrootBoolT hpropArgT hprefix hpropS'' =>
          ...
          | app hrootIteT hboolArgT hrootIteS' hboolS =>
            ...
            have hselector : env.IsDefEqU 0 []
              ...
              cases hab : (a == b) with
              | false =>
                simpa [hab, VExpr.boolLit] using VEnv.reflectionITE_false wf
                  hrtypeClosed hrtypeClosed hriteClosed hcertCanon.2
                  hpropT (by simpa [hab, VExpr.boolLit] using hproofLitT)
                  (by simpa [hab, VExpr.boolLit] using hproofLitT)
                  hboolT htrueT hfalseT
              | true =>
                simpa [hab, VExpr.boolLit] using VEnv.reflectionITE_true wf
...
theorem Condition.natEq.checkForPrimitive.WF.semantic
  ...
  have hevidence : ∀ e ∈ Condition.natEqEvidenceExpressions,
    ...
    intro e he
    -- Prove `FVarsIn` structurally rather than deciding `hasFVar`/`hasMVar`:
    -- those read the cached `Expr.data` word built by the extern `mkData`,
    -- which the kernel cannot reduce, whereas `FVarsIn` recurses on
    -- constructors and reduces fine.
    have hclosed : ∀ e ∈ Condition.natEqEvidenceExpressions,
      ...
      simp [Condition.natEqEvidenceExpressions, Condition.natEq,
        ...
        Expr.FVarsIn, FVarsIn, Level.hasMVar',
  ...
  have hrtypeUnique : TrExprS.IsUnique Reflection.defn₂.type := by
    simp [TrExprS.IsUnique, Reflection.defn₂]
  ...
  have hrtypeCanon' : c.TrExprS q(Prop → Bool → Prop)
    ...
    exact TrExprS.propBoolPropType_of_contains
      c.Ewf c.hasPrimitives hbool c.lparams []
  ...
  have hdecideTy' : c.TrExprS q(Nat → Nat → Bool)
    ...
    exact TrExprS.natBinaryBoolType_of_contains
      c.Ewf c.hasPrimitives hnat hbool c.lparams []
...
/-- A successful Boolean-condition check retains the selector's typing and
the two branch equations used by the semantic reflection wrapper. -/
theorem Condition.bool.check.WF
    {c : TypeChecker.VContext} {s : TypeChecker.VState}
    {fail : ∀ {α}, TypeChecker.M α}
    ...
    (hfail : ∀ {α} {s'}, TypeChecker.M.WF c s'
      (fail : TypeChecker.M α) fun _ _ => False) :
    TypeChecker.M.WF c s (Condition.bool.check fail (ite := true)) fun _ _ =>
      c.HasType natITE' natITETy' ∧
      c.IsDefEqU trueL' trueR' ∧ c.IsDefEqU falseL' falseR' := by
  simp [Condition.check, Condition.bool]
  refine (TypeChecker.checkType.WF hdec.fvarsIn).bind fun _ _ _ _ => ?_
  refine inferTypeIsDefEqGuard.bind_WF hprop hpropTy hfail fun _ _ => ?_
  refine checkTypeIsDefEqGuard.bind_WF
    hnatITE hnatITE_unique hnatITETy hfail fun _ hnatITEHas => ?_
  refine isDefEqGuard.bind_WF (e₂' := trueR')
    htrueL ?_ hfail fun _ htrueEq => ?_
  · exact TrExprS.of_exprShapeEq (v := trueR')
      (exprShapeEq_sound (by rfl)) htrueR
  exact (isDefEqGuard.WF (e₂' := falseR') hfalseL
    (TrExprS.of_exprShapeEq (v := falseR')
      (exprShapeEq_sound (by rfl)) hfalseR)
    hfail).mono fun _ _ _ hfalseEq =>
...
theorem Condition.bool.check.WF.reflects
    ...
    (hcheck : TypeChecker.M.WF c s
      (Condition.bool.check fail (ite := true)) fun _ _ =>
        c.HasType ite
...
theorem Condition.bool.checkForPrimitive.WF.semantic
  ...
  have hevidence : ∀ e ∈ Condition.boolEvidenceExpressions,
      e.FVarsIn (· ∈ c.vlctx.fvars) := by
    intro e he
    -- As above: `FVarsIn` reduces where `hasFVar`/`hasMVar` cannot.
    have hclosed : ∀ e ∈ Condition.boolEvidenceExpressions,
        e.FVarsIn (· ∈ c.vlctx.fvars) := by
      simp [Condition.boolEvidenceExpressions, Condition.bool,
        Condition.boolNatITE, Expr.FVarsIn, FVarsIn, Level.hasMVar',
        Expr.lam0, mkApp2, mkApp]
    exact hclosed e he
  ...
  have hiteTy' : c.TrExprS q(Bool → Nat → Nat → Nat)
    ...
    exact TrExprS.boolNatBinaryType_of_contains
      c.Ewf c.hasPrimitives hbool hnat c.lparams []
...
theorem checkPrimitiveDef.natBitwise_eq (hname : v.name = ``Nat.bitwise) :
...
set_option maxHeartbeats 800000 in
theorem checkNatBitwisePrimitive.WF_typed
    {c : TypeChecker.VContext} {s : TypeChecker.VState}
    {v : DefinitionVal} {ty' : VExpr}
    (hcparams : c.lparams = v.levelParams) (hvlctx : c.vlctx = [])
    (hty : c.TrExprS v.type ty') :
    TypeChecker.M.WF c s
      (checkNatBitwisePrimitive c.env v
        (throw <| .other s!"invalid form for primitive def {v.name}"))
      fun cert _ => ∃ ite decide,
        v.levelParams = [] ∧
        c.venv.contains ``Bool ∧ c.venv.contains ``Nat ∧
        c.venv.contains ``Nat.beq ∧ c.venv.contains ``Nat.add ∧
        c.venv.contains ``Nat.mod ∧ c.venv.contains ``Nat.div ∧
        c.venv.IsDefEqU c.lparams.length [] ty'
          (.forallE (.forallE .bool <| .forallE .bool .bool) <|
            .forallE .nat <| .forallE .nat .nat) ∧
        cert.NormalizedValid c v.value ∧
        c.TrExprS Condition.bool.boolNatITE ite ∧
        c.venv.ReflectsBoolNatITE ite ∧
        c.TrExprS Condition.natEqDecideFn decide ∧
        VEnv.ReflectsNatEqDecide c.venv decide := by
  unfold checkNatBitwisePrimitive
  dsimp only
  ...
  · rw [if_pos hdeps]
    ...
    have contains (n : Name) (hsrc : c.env.contains n = true)
        ...
        c.venv.contains n :=
      VContext.contains_safe_primitive c hsrc hprim
    ...
    have hnat0 := TrExprS.natType_of_contains
      c.Ewf c.hasPrimitives hnatC c.lparams []
    ...
    exact (isDefEq.WF hty hcanon).bind fun b _ _ htyEq => by
      ...
      · rw [if_pos hb]
        ...
        | some equation =>
          ...
          by_cases hall : es.all (fun e =>
              exprLooseBVarRange e == 0 && !e.hasFVar && !e.hasMVar) = true
          · rw [if_pos hall]
            have hes : ∀ e ∈ es,
                e.FVarsIn (· ∈ c.vlctx.fvars) := by
              intro e he
              ...
              rcases hh with ⟨⟨_, hnoFVar⟩, hnoMVar⟩
              have hnoFVar' : e.hasFVar = false := by simpa using hnoFVar
              have hnoMVar' : e.hasMVar = false := by simpa using hnoMVar
              exact Expr.closed_fvarsIn hnoFVar' hnoMVar'
            ...
            exact (unfoldNatWellFoundedBoolNat2Cert.WF
              ...
              refine (TypeChecker.M.WF.sandbox_of_independent ?_).bind
                fun _ _ _ hresult => .pure hresult
              exact (inferType.WF hequation).bind fun _ _ _ hinfer => by
                ...
                exact (isDefEq.WF hequationTy hcanon).bind
                  ...
                  by_cases hq : q = true
                  · rw [if_pos hq]
                    exact (Condition.natEq.checkForPrimitive.WF.semantic
                      hclparams hvlctx hboolC hnatC hbeqC
                      (fun {_} {_} => .throw)).bind
                      fun _ _ _ hnatCert =>
                      (Condition.bool.checkForPrimitive.WF.semantic
                        hclparams hvlctx hboolC hnatC
                        (fun {_} {_} => .throw)).bind
                        fun _ _ _ hboolCert =>
                                    ...
                                    exact (isDefEq.WF hs₁ hs₂).bind
                                      ...
                                      · rw [if_pos hsQ]
                                        exact (checkNatBitwiseZero.WF
                                          ...
                                          (fun {_} {_} => .throw)).mono
                                          fun _ _ _ _ => by
                                          rcases hnatCert with
                                            ⟨decide, hdecideS, _, hdecide⟩
                                          rcases hboolCert with
                                            ⟨ite, hiteS, hite⟩
                                          exact ⟨ite, decide, hlevels, hboolC,
                                            hnatC, hbeqC, haddC, hmodC,
                                            hdivC, htyEq0,
...
theorem checkPrimitiveDef.natBitwise.WF_typed
  ...
  rw [checkPrimitiveDef.natBitwise_eq hname]
  refine getEnv.WF.bind ?_
  intro _ _ _ ⟨rfl, rfl⟩
  exact (checkNatBitwisePrimitive.WF_typed hcparams hvlctx hty).bind
